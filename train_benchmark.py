...
Both features require:
  ...
  • data/<dataset>/meta.pkl containing 'stoi' and 'itos' maps
    (standard output of a character-level prepare.py).

Usage examples:
  # Train normally (no benchmarking):
  python train_benchmark.py config/my_config.py
...
# -----------------------------------------------------------------------------
# comp560 benchmarking / eval  (all disabled by default — zero impact on existing runs)
separator_token          = '='     # string that separates input from output in each sequence
...
# ---------------------------------------------------------------------------
# Pre-training-loop benchmarking setup
# ---------------------------------------------------------------------------
_early_stop    = False   # set to True inside eval block to break the training loop
perfect_tf_iter = None   # iteration when TF exact-match first hit 100 %
# dict: split -> List[dict], e.g. {'val': [...]} or {'train': [...], 'val': [...]}
tf_eval_datasets = {}

if enable_tf_eval and master_process:
    _tf_splits = ['train', 'val'] if benchmark_target == 'both' else [benchmark_target]
    _tf_max    = tf_eval_max_samples if tf_eval_max_samples > 0 else None
    for _tf_split in _tf_splits:
        _tf_jsonl = os.path.join(data_dir, f'{_tf_split}.jsonl')
        tf_eval_datasets[_tf_split] = comp560ext.load_eval_dataset(_tf_jsonl, max_samples=_tf_max)
        print(f"TF eval: loaded {len(tf_eval_datasets[_tf_split])} samples from {_tf_jsonl}")
# ---------------------------------------------------------------------------
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        # -----------------------------------------------------------------------
        # Fast TF Exact Match Evaluation
        # -----------------------------------------------------------------------
        if enable_tf_eval and tf_eval_datasets:
            tf_results = {}  # split -> (acc, em, total)
            for _tf_split, _tf_data in tf_eval_datasets.items():
                tf_results[_tf_split] = comp560ext.run_tf_eval(
                    raw_model, _tf_data, encode,
                    separator_token, stop_token,
                    device, ctx,
                )
                tf_acc, tf_em, tf_total = tf_results[_tf_split]
                print(f"step {iter_num}: TF exact-match [{_tf_split}] {tf_acc:.1f}% ({tf_em}/{tf_total})")
                if wandb_log:
                    wandb.log({"iter": iter_num, f"tf_exact_match/{_tf_split}": tf_acc})
            # Early-stop is driven by 'val' if available, otherwise the only loaded split.
            _ref_split = 'val' if 'val' in tf_results else next(iter(tf_results))
            _ref_acc   = tf_results[_ref_split][0]
            if _ref_acc == 100.0 and perfect_tf_iter is None:
                perfect_tf_iter = iter_num
                print(f"*** TF 100% exact-match [{_ref_split}] first reached at iter {iter_num} ***")
                if early_stop_on_perfect_tf:
                    _early_stop = True
        # -----------------------------------------------------------------------

    # Early-stop requested by TF eval (checked outside the eval if-block so that
    # iter_num == 0 / eval_only logic below still executes in its normal position).
    if _early_stop:
        print("early_stop_on_perfect_tf=True — halting training.")
        break
